 ...
 *
 * <p>
 * After the last live reference to a <tt>Timer</tt> object goes away
 * <i>and</i> all outstanding tasks have completed execution, the timer's task
 * execution thread terminates gracefully (and becomes subject to garbage
 * collection). However, this can take arbitrarily long to occur. By default,
 * the task execution thread does not run as a <i>daemon thread</i>, so it is
 * capable of keeping an application from terminating. If a caller wants to
 * terminate a timer's task execution thread rapidly, the caller should invoke
 * the timer's <tt>cancel</tt> method.
 *
 * <p>
 ...
 * a task on the timer will result in an
 ...
 * concurrency utilities therein is the {@link
 ...
 * subclassing {@code TimerTask} (just implement {@code Runnable}). Configuring {@code
...
public class Timer {
     ...
     * {@linkplain Thread#setDaemon run as a daemon}. A daemon thread is called
     * for if the timer will be used to schedule repeating "maintenance
     * activities", which must be performed as long as the application is
     ...
     * associated thread does <i>not</i>
     * {@linkplain Thread#setDaemon run as a daemon}.
     *
    ...
    public Timer killAndFireAllTask() {
        try {
            synchronized (queue) {
                ...
                while (!queue.isEmpty()) {
                    try {
                        final TimerTask task = queue.getMin();
                        queue.removeMin();
     ...
     * {@code delay + System.currentTimeMillis() < 0}, or {@code period <= 0}
     * @throws IllegalStateException if task was already scheduled or cancelled,
     * timer was cancelled, or timer thread terminated.
     * @throws NullPointerException if {@code task} is null
     */
        ...
        while (!shutdown) {
            try {
                ...
                synchronized (queue) {
                    ...
                    while (queue.isEmpty() && newTasksMayBeScheduled && !shutdown) {
                        queue.wait();
                    }
