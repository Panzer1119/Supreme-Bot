...
public class GuildMemberLogger extends ListenerAdapter {
    ...
    public final void onGuildMemberJoin(GuildMemberJoinEvent event) {
        ...
        Standard.log(timestamp, event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_JOIN, "[%1$s] [%2$s] %3$s joined this Guild", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention());
    }

    private final void onGuildMemberJoinKicked(Instant timestamp, GuildMemberJoinEvent event) {
        Standard.log(timestamp, event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_JOIN_KICKED, "[%1$s] [%2$s] %3$s joined this Guild, but was directly kicked", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention());
    }

    @Override
    public final void onGuildMemberLeave(GuildMemberLeaveEvent event) {
        Standard.log(Instant.now(), event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_LEAVE, "[%1$s] [%2$s] %3$s left this Guild", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention());
    }
    ...
    public final void onGuildMemberRoleAdd(GuildMemberRoleAddEvent event) {
        ...
        Standard.log(timestamp, event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_ROLE_ADD, "[%1$s] [%2$s] %3$s got added %4$s", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), Util.rolesToString(event.getRoles(), advancedGuild.getSettings().getProperty(LOG_MEMBER_ROLES_ASMENTION, false)));
    }
    ...
    public final void onGuildMemberRoleRemove(GuildMemberRoleRemoveEvent event) {
        final Instant timestamp = Instant.now();
        final AdvancedGuild advancedGuild = Standard.getAdvancedGuild(event.getGuild());
        Standard.log(timestamp, event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_ROLE_REMOVE, "[%1$s] [%2$s] %3$s got removed %4$s", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), Util.rolesToString(event.getRoles(), advancedGuild.getSettings().getProperty(LOG_MEMBER_ROLES_ASMENTION, false)));
    }

    @Override
    public final void onGuildMemberNickChange(GuildMemberNickChangeEvent event) {
        Standard.log(Instant.now(), event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_MEMBER, LOG_TEXT_MEMBER_NICK_CHANGE, "[%1$s] [%2$s] %3$s changed his Nickname from \"%4$s\" to \"%5$s\"", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), event.getPrevNick(), event.getNewNick());
    }
