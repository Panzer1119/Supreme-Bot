...
public class GuildVoiceLogger extends ListenerAdapter {
    ...
    public final void onGuildVoiceJoin(GuildVoiceJoinEvent event) {
        Standard.log(Instant.now(), event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_VOICE, LOG_TEXT_VOICE_JOIN, "[%1$s] [%2$s] %3$s joined #%4$s", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), event.getChannelJoined().getName());
    }

    @Override
    public final void onGuildVoiceMove(GuildVoiceMoveEvent event) {
        Standard.log(Instant.now(), event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_VOICE, LOG_TEXT_VOICE_MOVE, "[%1$s] [%2$s] %3$s moved from #%4$s to #%5$s", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), event.getChannelLeft().getName(), event.getChannelJoined().getName());
    }

    @Override
    public final void onGuildVoiceLeave(GuildVoiceLeaveEvent event) {
        Standard.log(Instant.now(), event.getGuild(), LOG_NAME, LOG_CHANNEL_ID_VOICE, LOG_TEXT_VOICE_LEAVE, "[%1$s] [%2$s] %3$s left #%4$s", LOG_DATE_TIME_FORMAT, event.getMember().getAsMention(), event.getChannelLeft().getName());
    }
